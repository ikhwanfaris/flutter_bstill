...
class StaggeredMenuBarState extends State<StaggeredMenuBar> {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        title != null
            ? Text(title!, style: TextStyles.bodyExtraSm.subTitleColor.semiBold)
                .bP16
            : Container(),
        Wrap(
          spacing: 5,
                  ...
                  children: [
                    ...
                    Text(menus[index],
                        style: index == selectedMenuIndex
                            ? selectedTextStyle ??
                                TextStyles.body.bold.themeColor
                            : textStyle ?? TextStyles.body.semiBold),
...
class HorizontalSegmentBarWithPointerState
  ...
  List<String> menus = ['Detail', 'Related'];
  // List<String> menus = [AppLocalizations.of(context)?.detail, AppLocalizations.of(context)?.related];
  int selectedMenuIndex = 0;
  late TextStyle? textStyle;
  late TextStyle? selectedTextStyle;
  late double width;
